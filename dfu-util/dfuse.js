...
(function() {
    ...
    dfuse.Device.prototype.do_download = async function(xfer_size, data, manifestationTolerant) {
        ...
        try {
            ...
            // Device may have rebooted after manifestation, ignore getStatus errors
            if (error.includes("GETSTATUS") || error.includes("getStatus") || error.includes("ControlTransferIn")) {
                this.logDebug("Device may have rebooted after manifestation (ignoring status check error)");
            } else {
                this.logError(error);
            }
        }
