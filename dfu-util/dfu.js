...
(function() {
    ...
    dfu.Device.prototype.getStatus = function() {
        return this.requestIn(dfu.GETSTATUS, 6).then(
            ...
            error => {
                // Check if device is disconnected (normal after firmware update)
                if (this.disconnected || (error && (error.includes("disconnect") || error.includes("NetworkError")))) {
                    // Silently ignore if device is disconnected (expected after firmware update)
                    return Promise.reject("Device disconnected (expected after firmware update)");
                }
    ...
    dfu.Device.prototype.poll_until = async function(state_predicate) {
        ...
        while (retryCount < maxRetries) {
            try {
                ...
                this.logWarning("getStatus failed, retrying (" + retryCount + "/" + maxRetries + ")...");
        ...
        while (!state_predicate(dfu_status.state) && dfu_status.state != dfu.dfuERROR) {
            ...
            while (retryCount < maxRetries) {
                try {
                    ...
                    // If device disconnected (expected after firmware update), exit gracefully
                    if (error.includes("disconnected") || error.includes("expected after firmware update") || 
                        this.disconnected || (error.includes("NetworkError") && error.includes("transfer error"))) {
                        // Device likely rebooted after firmware update, this is normal
                        this.logDebug("Device disconnected during poll (likely rebooted after firmware update)");
                        // Return current status or a default status indicating disconnection
                        return dfu_status;
                    }
                    retryCount++;
                    if (retryCount >= maxRetries) {
                        throw "Failed to get status during poll: " + error;
                    }
                    this.logWarning("getStatus failed during poll, retrying (" + retryCount + "/" + maxRetries + ")...");
                    await new Promise(resolve => setTimeout(resolve, 100));
                }
            }
        }
